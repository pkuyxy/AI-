...
def create_interface():
    ...
    with gr.Blocks(theme=gr.themes.Default(primary_hue="indigo"), title="AI Chat") as demo:
        ...
        # 密钥状态提示和配置面板
        with gr.Column(visible=not load_secrets()) as key_config_panel:
            gr.Markdown("### API密钥配置")
        ...
        def use_default_keys_action():
            """使用默认密钥并返回状态信息"""
            global using_default_keys
            success = load_secrets(use_default=True)
            if success:
                msg = "⚠️ 正在使用共享API密钥（功能受限）"
                print("\n" + msg + "\n")
                return gr.update(value=msg, visible=True), gr.update(visible=False), gr.update(visible=True), gr.update(value=msg)
            return gr.update(value="❌ 无法使用默认密钥", visible=True), gr.update(visible=False), gr.update(visible=False), gr.update(value="")
        
        # 事件绑定
        demo.load(
            fn=lambda: (gr.update(visible=not load_secrets()), gr.update(visible=using_default_keys), gr.update(value="⚠️ 正在使用共享API密钥（功能受限）" if using_default_keys else "")),
